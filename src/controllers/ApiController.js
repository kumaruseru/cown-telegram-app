...
class ApiController extends BaseController {
    ...
    setupRoutes() {
        // Telegram operations
        this.registerRoute('post', '/api/telegram/connect', this.connectTelegram.bind(this), [this.requireAuth.bind(this)]);
        this.registerRoute('get', '/api/telegram/status', this.getTelegramStatus.bind(this), [this.requireAuth.bind(this)]);
        this.registerRoute('post', '/api/telegram/disconnect', this.disconnectTelegram.bind(this), [this.requireAuth.bind(this)]);
        
        // Chat operations
        this.registerRoute('get', '/api/chats', this.getChats.bind(this), [this.requireAuth.bind(this)]);
        this.registerRoute('get', '/api/chats/:chatId/messages', this.getChatMessages.bind(this), [this.requireAuth.bind(this)]);
        this.registerRoute('post', '/api/chats/:chatId/send', this.sendMessage.bind(this), [this.requireAuth.bind(this)]);
        
        // User management
        this.registerRoute('put', '/api/user/profile', this.updateProfile.bind(this), [this.requireAuth.bind(this)]);
        this.registerRoute('post', '/api/user/change-phone', this.changePhone.bind(this), [this.requireAuth.bind(this)]);
        
        // System operations
        this.registerRoute('get', '/api/system/stats', this.getSystemStats.bind(this), [this.requireAuth.bind(this), this.requireAdmin.bind(this)]);
    }
    ...
    async connectTelegram(req, res) {
        try {
            ...
            const result = await telegramService.connectUser(userId, { phone, code, password });
    ...
    async disconnectTelegram(req, res) {
        try {
            ...
            return this.sendError(res, 'Failed to disconnect from Telegram', 500);
        }
    }
    ...
    async getChats(req, res) {
        try {
            ...
            const telegramService = this.getService('telegram');
            const chats = await telegramService.getUserChats(userId, { limit, offset });
    ...
    async getChatMessages(req, res) {
        try {
            ...
            const telegramService = this.getService('telegram');
            const messages = await telegramService.getChatMessages(userId, chatId, { limit, offset });
    ...
    async sendMessage(req, res) {
        try {
            ...
            if (result.success) {
                return this.sendSuccess(res, result.data, 'Message sent successfully');
            } else {
                return this.sendError(res, result.message, 400);
            }
        } catch (error) {
    ...
    async changePhone(req, res) {
        try {
            ...
            const otpVerification = await otpService.verifyOTP(normalizedPhone, otpCode, 'phone');
            ...
            if (existingUser && existingUser.id !== userId) {
                return this.sendError(res, 'Phone number is already in use', 400);
            }
