...
async function testTelegramAuth() {
    ...
    try {
        ...
        const webhookResponse = await axios.post('http://localhost:3001/webhook/telegram', {
            update_id: Date.now(),
            message: mockMessage
        });
        ...
        setTimeout(async () => {
            try {
                const statusResponse = await axios.get('http://localhost:3001/auth/telegram/status');
