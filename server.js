...
async function main() {
    ...
    try {
        ...
        console.log(
            '📊 Health Check: http://localhost:' + app.config.port + '/health'
        );
        console.log(
            'ℹ️  App Info: http://localhost:' + app.config.port + '/info'
        );
    } catch (error) {
...
if (currentNodeVersion < requiredNodeVersion) {
    console.error(
        `❌ Node.js version ${requiredNodeVersion} or higher is required. Current version: ${process.version}`
    );
    process.exit(1);
}
